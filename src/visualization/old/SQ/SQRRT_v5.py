...
class Arm:
    ...
    def add2Extras(self, cx, cy, cz):
        blob = SuperQuadric(0.5,0.5,0.5)
        blob.transform(translation=[cx,cy,cz])
        self.extras.append(blob)
        print(self.extras)
    ...
    def plotArm(self):
        ...
        for extra in self.extras:
            mlab.mesh(extra.W[0], extra.W[1], extra.W[2])
#            if primitive == self.links[0]:
                #give the base SQ axes and outline
            mlab.axes()
            mlab.outline()
...
if __name__ == '__main__':
    ...
    for i in range(6):
        start = time.time()
        RobotArm.newStandardLink()#i*1.5, i, i*4, i, i)
        end = time.time()
    ...
    # RobotArm.add2Extras(9,0,12)
    # RobotArm.add2Extras(58,5,-1)
